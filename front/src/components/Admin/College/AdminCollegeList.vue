  ...
  <div style="display: flex;flex-wrap: wrap;justify-content: center">
    <div class="cart_box">
      <van-grid column-num="2">
        <van-grid-item text="学院名" />
        <van-grid-item text="学院ID" />
      </van-grid>
        ...
        >
          <div v-for="(item,i) in list" :key="i" style="display: flex;flex-wrap: wrap;justify-content: center">
            <van-swipe-cell style="width: 100%">
              <van-cell title-class="van-ellipsis" :border="false" :title="item['collegeName']" :value="item['collegeId']" icon="smile-comment-o"/>
              <template #right>
                <van-button square text="修改" type="primary" @click="editCollege(i)" />
                <van-button square text="删除" type="danger" @click="deleteCollege(i)" />
              </template>
            </van-swipe-cell>
          </div>
        </van-list>
    ...
    onLoad() {
      setTimeout(() => {
        ...
        if (this.list.length >= this.listMaxSize) {
          ...
          //获取数据
          apiGet('college',this.getData).then((resp)=>{
    ...
    deleteCollege(index){
      Dialog.confirm({
        ...
        message: '确定要删除这个学院吗',
          ...
          .then(() => {
            apiDelete('college',{id:this.list[index]['collegeId']}).then((resp)=>{
              if(resp.data['code']===0){
