<template>
  <div>
    <van-skeleton :loading="loading" title :row="16" />
    <van-cell-group inset class="info_card" v-show="!loading">
      ...
      <van-cell>
        ...
        <van-button style="width: 100%;margin-top: 10px;margin-bottom: 10px" round type="danger" @click="out">登出</van-button>
      </van-cell>
    </van-cell-group>
...
export default {
  ...
  beforeMount() {
    try{
      this.getParam["id"]=localStorage.getItem("userId")
    }catch (e){
      Notify({ type: 'warning', message: '未找到用户id' })
      this.out()
    }
    apiGet("user",this.getParam).then((resp)=>{
